...
def get_parser_class(input_type: str, filename: str) -> Type[BaseParser]:
    ...
    if not any(filename.endswith(ext) for ext in supported_extensions):
        raise ValueError(
            f"Unsupported file extension for `{filename}` in mode `{input_type}`. {supported_extensions=}"
        )
...
def convert_file(
    ...
    **kwargs,
    ...
    converter = parser_class(
        file, filetype, process_raw_image_urls, image_processor_key
    )
